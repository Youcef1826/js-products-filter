...
const search = document.getElementById("search"); // Input search HTML element
const selectCategory = document.getElementById("select-category"); // Select HTML element
const sort = document.getElementById("sort"); // Sort element lower to highest price


for (let category of productCategory) { // Loop to find all products categories object

    selectCategory.innerHTML += `<option value="${category}">${category}</option>`; // Display options on select element
};


const displayProducts = (products) => { // Function to display products

    productsList.innerHTML = "";

    for (const product of products) { // Loop to find all products on products object
        ...
        `;
    }; // Display product with html and css
};
displayProducts(products);


const filter = () => { // Function to filtered products with search, select and sort
    
    let filteredProducts = products;

    if (search.value) { // Search element

        filteredProducts = filteredProducts.filter((product) => product.title.toLowerCase().includes(search.value.toLowerCase()));
    };

    if (selectCategory.value) { // Select element
        
        filteredProducts = filteredProducts.filter((product) => product.category === selectCategory.value);
    };

    if (sort.value === "cheapest") { // Sort element
...
selectCategory.onchange = filter;
